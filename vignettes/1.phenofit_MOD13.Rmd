...
d     <- df[site == sites[7], ] # get the first site data
...
```{r}
ratio <- 1.8
CairoPDF("phenofit_10st_v2.pdf", 10*ratio, 6*ratio)
fits <- dlply(df, .(site), function(d){
    fit <- NA
    tryCatch({
...
ratio <- 1.8
if (IsPlot) CairoPDF("phenofit_10st.pdf", 10*ratio, 6*ratio)
fits <- dlply(df, .(site), function(d){
    fit <- NA
    tryCatch({
        south <- d$lat[1] < 0
