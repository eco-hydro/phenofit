...
if ('SummaryQA' %in% colnames(dt)){
    dt$SummaryQA %<>% factor() %>% mapvalues(c("0", "1", "2", "3"), c(" good", " margin", " snow&ice", " cloud"))
}
...
plot_phenofit(fit, d) %>% grid.draw()# plot to check the curve fitting
...
temp <- ExtractPheno(fit$fits$ELMORE[1:5], IsPlot = T)
