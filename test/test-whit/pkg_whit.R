...
df <- lst$MOD13A1
df <- merge(df, stations212[, .(site, lat, IGBP)], by = "site")
...
info <- df[, .(mean = mean(y, na.rm = T),
               sd = sd(y, na.rm = T),
               kurtosis = kurtosis(y, na.rm = T, type = 2),
               skewness = skewness(y, na.rm = T, type = 2)), .(site)]
x <- merge(data.table(x), info, by = "site")
...
lm1 <- lm(lambda~kurtosis+mean+sd+skewness, x) #
summary(lm1)
slm1 <- stepAIC(lm1)
summary(slm1)
slm1$anova

lambda <- 1.7916*mean + -3.8382*sd + 0.7335
par(mfrow = c(4, 4))
...
plot(slm1$fitted.values, x$lambda); grid()
abline(a = 0, b = 1, col = "red", lwd = 1.5)
