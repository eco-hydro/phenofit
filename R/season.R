                   ...
                   ...)
{
    ...
    nyear <- round(npt/nptperyear) # Needn't to ceil, matter most for parameter adjustment
    ...
    while (iloop <= 3){
        ...
        info_peak = findpeaks_season(ypred, r_max*A, r_min*0, minpeakdistance, ypeak_min, nyear, nups = nups)
    ...
    if (rm.closed) {
        pos   <- adjust_pos(pos, rm.closed, ypred, y_min = r_min*A, minpeakdistance)
        pos$t <- t[pos$pos]
