...
files <- dir("V:/result/gee_whittaker", "*.RDS", full.names = T) %>%
...
check_roughtFitting <- function(x, prefix){
    ...
    d_diff <- data.table(index = index_all, dmax = dmax_all)

    ## 2. visualization
    d2 <- merge(df_info2, d_diff)
    d2[, diff := value - wWH]

    # a <- d2[type == "part" & meth == "wWH2" & index == "R2" & diff > 0.05]
    ...
    d_dominant <- ddply_dt(d2, .(table(kind)), .(meth, index, type, label))
...
fun_Figure9 <- function(pdat, d_dominant, typename = "part", prefix = Sys.Date()){
    d_dom  <- d_dominant[type == typename & meth == "wWH2"]
    pdati  <- pdat[type == typename & meth == "wWH2"]

    p <- ggplot(pdati, aes(wWH, value, color = kind)) +
